...
Expanding the model to a ==general polynomial of degree $m$== gives ==$y_{i}=a_{0}+a_{1}x_{i}+a_{2}x_{i}^{2}+\dots+a_{m}x_{i}^{m}+e$==, and we once again ==minimise the sum of squared residuals== $S_{r}=\sum_{i=0}^{n-1}(y_{i}-a_{0}-a_{1}x_{i}-a_{2}x_{i}^{2}-\dots-a_{m}x_{i}^{m})^2$ to obtain the best-fitting polynomial coefficients.
