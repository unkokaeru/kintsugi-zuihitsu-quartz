...
confirming $T_{ij}$ is a rank-2 tensor. $\blacksquare$
...
$$
A'_{ij} = L_{ik} L_{jm} A_{km} = L_{ik} L_{jm} A_{mk} = L_{jm} L_{ik} A_{mk} = A'_{ji}. \quad \blacksquare
$$
...
Repeating for $i=2$ and $i=3$ gives $T_{12} = T_{21}$ and $T_{13} = T_{31}$, so $T_{ij} = T_{ji}$. $\blacksquare$
...
- **$\varepsilon_{ijk}T_{jk}=0$ example**: fixing each $i$ and reading off non-zero $\varepsilon$ components forces $T_{23}=T_{32}$, $T_{12}=T_{21}$, $T_{13}=T_{31}$ — so $T_{ij}$ is symmetric; geometrically, $\varepsilon$ only sees the antisymmetric part
