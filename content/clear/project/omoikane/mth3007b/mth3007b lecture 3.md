...
Which can be specified with the four unknowns, $a_1, a_2, p_1, q_{11}$. We can find these by using a Taylor expansion on the general second order Runge-Kutta form, $y_{i+1} \approx y_i + \Delta t \cdot \phi(t_i, y_i, \Delta t)$, using the aforementioned increment function.

This will give various solutions, as there are only 
...
**Taylor-expanding the ODE** $y'(t) = g(t, y(t))$ gives 

$$y_{i+1} = y_i + \Delta t\, g_i + \frac{\Delta t^2}{2!}\left(\frac{\partial g_i}{\partial t_i} + \frac{\partial g_i}{\partial y_i} g(t_i, y_i)\right) + \mathcal{O}(\Delta t^3)$$

Note that the total derivative $g' = \partial g/\partial t + (\partial g/\partial y)\dot{y}$ is required because $y$ also changes with $t$. 

**Taylor-expanding $k_2$** in both arguments gives 

$$k_2 = g(t_i, y_i) + p_1 \Delta t \frac{\partial g_i}{\partial t_i} + q_{11}\Delta t\, k_1 \frac{\partial g_i}{\partial y_i} + \mathcal{O}(\Delta t^2)$$

Substituting back into the RK2 scheme and equating powers of $\Delta t$ with the ODE Taylor expansion yields three equations 

$$a_1 + a_2 = 1, \qquad a_2 p_1 = \tfrac{1}{2}, \qquad a_2 q_{11} = \tfrac{1}{2}$$

Since there are **four unknowns and only three equations**, one parameter is free — giving infinitely many valid 2nd-order RK methods. Choosing $a_2$ freely, the remaining coefficients are 

$$a_1 = 1 - a_2, \qquad p_1 = q_{11} = \frac{1}{2a_2}$$
...
The most widely used Runge-Kutta method is **RK4**, a 4th-order method with global truncation error $\mathcal{O}(\Delta t^4)$: 

$$y_{i+1} \approx y_i + \frac{\Delta t}{6}(k_1 + 2k_2 + 2k_3 + k_4)$$
...
> [!warning] All explicit RK methods are **not** symmetric, and therefore all suffer from long-time drift. 

### Implicit Trapezoid Method

For the ODE $\dot{y}(t) = g(t, y(t))$, the **implicit trapezoid method** is 

$$y_{i+1} \approx y_i + \frac{\Delta t}{2}\!\left(g(t_i, y_i) + g(t_{i+1}, y_{i+1})\right)$$

It is **implicit** since $y_{i+1}$ appears on both sides — this equation must be rearranged to isolate $y_{i+1}$ before implementation, analogously to the implicit Euler method. It is 2nd-order, with global truncation error $\mathcal{O}(\Delta t^2)$. 
