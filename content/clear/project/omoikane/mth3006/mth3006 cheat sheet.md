...
**Types**: Fredholm (fixed limits $a,b$) vs Volterra (upper limit $x$); 1st kind ($y$ only inside integral) vs 2nd kind ($y$ inside and outside).
...
**Formulas**: $\tilde{f}(\omega) = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty} f(t)e^{-i\omega t}dt$ (full), $\quad\tilde{f}_s = \sqrt{\frac{2}{\pi}}\int_0^{\infty} f\sin(\omega t)dt$ (odd $f$ on $[0,\infty)$), $\quad\tilde{f}_c = \sqrt{\frac{2}{\pi}}\int_0^{\infty} f\cos(\omega t)dt$ (even $f$ on $[0,\infty)$).

1. **Method**: Write formula → split integral by ranges → evaluate. Use Euler ($e^{-i\omega t}=\cos\omega t - i\sin\omega t$) when $f$ has trig functions.
2. **Absolute value** $|t|$: $|t|=\begin{cases}-t & t<0 \\ t & t\geq 0\end{cases}$ → split at 0. E.g., $\text{FT}\{e^{-a|t|}\} = \sqrt{\frac{2}{\pi}}\frac{a}{a^2+\omega^2}$.
3. **Delta functions**: Sifting $\int\delta(t-a)g(t)dt=g(a)$. E.g., $\text{FT}\{\delta(t-a)+\delta(t+a)\}=\sqrt{\frac{2}{\pi}}\cos(a\omega)$.
