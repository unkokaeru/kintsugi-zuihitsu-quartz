...
- **Integration by Parts**: When integrating a product of two functions, especially when one becomes simpler when differentiated…
	...
	7. Add constant $+ C$.
- **Integrating Factor** (for First-Order Linear ODEs): For first-order linear differential equations of the form $\frac{dy}{dx} + P(x)y = Q(x)$…
	1. Write the equation in standard form: $\frac{dy}{dx} + P(x)y = Q(x)$.
	2. Calculate the integrating factor: $\mu(x) = e^{\int P(x) dx}$ (no constant of integration needed here).
	3. Multiply both sides of the ODE by $\mu(x)$: $\mu(x) \frac{dy}{dx} + \mu(x) P(x) y = \mu(x) Q(x)$.
	4. Recognize the left side as the derivative of a product: $\frac{d}{dx}[\mu(x) y] = \mu(x) Q(x)$.
